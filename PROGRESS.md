...
3. **다음**: 제한된 7개 모델 pool에 맞게 MixInstruct 벤치마크 데이터셋 필터링 — `src/router/mix_instruct.py`의 `MixInstructOracle`이 `expert_names` 인자로 필터링 지원하는지 확인 필요
4. Query encoder(MLP) 학습 — `scripts/train_query_encoder.py --dataset mix-instruct --pool <7개 모델 json>`
