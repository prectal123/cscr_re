"""min(pct=0.3, K_CAP=3): caps the percentage-based selection so a query
with many positive candidates doesn't average over an unbounded number of
them (user's concern -- "30% 그룹이 너무 많아지면 안돼"). Per-row K =
min(ceil(0.3 * n_positives), 3, n_positives). Selection itself (sort/topk)
is a discrete op with no gradient through WHICH elements are chosen, but
that's the same as ordinary max-pooling -- gradient flows normally through
the VALUES of the selected elements, so this is fully differentiable
end-to-end, same as the existing min-pos/min-top-3/min-pct30 variants.

Same EmbedLLM V2 FP (80-dim, main established FP), same pct=0.3 catfilter
upstream, same fast cached pipeline as the K=3 and pct=0.3 tests.
"""
import json
import sys
import time
...
FP_DIR = Path("local_descriptors/embedllm-ceiling")
ANALYSIS_DIR = Path("local_descriptors/embedllm-analysis")
REFERENCE_NOTE = ("Ablation cell: min(pct=0.3,cap=3) WITHOUT catfilter (PCT_CATFILTER=1.0, keeps all positives). "
                   "Combined (with catfilter) = 0.5867; K=1+catfilter (min-pos alone) = 0.5587 (RESULTS_SUMMARY 2.3); "
                   "catfilter-alone = 0.5364; vanilla GRPO = 0.4802")
...
HOLDOUT_FRAC = 0.15
SEEDS = [0, 1, 2]
PCT_CATFILTER = 1.0  # ablation: no catfilter demotion (keeps all positive candidates)
PCT_MINLOSS = 0.3
K_CAP = 3
...
def minpctcap_loss(cos_sim, target, mask, pct=PCT_MINLOSS, k_cap=K_CAP):
    """min(pct * n_pos, k_cap): percentage-based like minpct_loss, but capped
    at k_cap so a query with many positives doesn't average over an
...
def main():
    ...
    E = np.stack([np.load(FP_DIR / f"{m}.npy") for m in models])
    E_t = torch.from_numpy(E).float().to(DEVICE)
    E_t = E_t / (E_t.norm(dim=1, keepdim=True) + 1e-9)
    E_norm = E / (np.linalg.norm(E, axis=1, keepdims=True) + 1e-12)
    print(f"FP dim={E.shape[1]}, dir={FP_DIR}", flush=True)
    ...
    results = []
    for seed in SEEDS:
        proj = train_fast(seed, cls_all, targets, masks, E_t, hidden_size, f"minpct{PCT_MINLOSS}cap{K_CAP}-nocatfilter-seed{seed}")
        with torch.no_grad():
    ...
    out_path = ANALYSIS_DIR / f"minpct{PCT_MINLOSS}cap{K_CAP}_nocatfilter_allseen_multiseed_results.json"
    json.dump(results, open(out_path, "w"), indent=2)

    audcs = [r["audc"] for r in results]
    print("\n" + "=" * 90)
    print(f"MIN-PCT-{PCT_MINLOSS}-CAP{K_CAP} (EmbedLLM V2 FP, catfilter pct={PCT_CATFILTER}, all-seen, {len(SEEDS)} seeds, {len(models)} models)")
    print("=" * 90)
    for r in results:
