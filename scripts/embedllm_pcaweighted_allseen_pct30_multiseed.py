"""Ceiling V1.5 (PCA-loading-weighted probe sampling, 1200 total probes vs
V1's uniform 1920) version of the confirmed all-seen combined (min-pos +
pct=0.3 catfilter) recipe -- this is the real test of the reallocation
idea: V1 (uniform N=24/category) showed a genuine, seed-consistent
degradation on all-seen (mean=0.5481 vs V2's 0.5652, -3.0%), even though it
held up perfectly on unseen. Does smarter (PCA-importance-weighted, fewer
total) probe allocation close that gap, using EVEN FEWER probes than V1?

On unseen, V1.5 already matched/slightly exceeded V1 and V2 (mean=0.5238
vs V1's 0.5171 and V2's ~0.517, though within 1 std -- not a clear win,
but "no worse with 37.5% less data" already held).

Model roster: embedllm-ceiling-pcaweighted-pca5/ was built from the FULL
train.csv model list (112 models, same universe as V2), so after the
standard EXCLUDE (phantom model), this uses the identical 111-model roster
as V2's all-seen results -- a clean, fully matched comparison (no roster
caveat like the probeN24 all-seen run had).

Reuses embedllm_pct30_combined_multiseed.py's exact training/eval code,
just swaps the FP source directory.
"""
import json
import sys
import time
...
PCA5_DIR = Path("local_descriptors/embedllm-ceiling-pcaweighted-pca5")  # 1800-probe, floor=15 version
ANALYSIS_DIR = Path("local_descriptors/embedllm-analysis")
REFERENCE_NOTE = "V2 all-seen (111 models, full data) = 0.5652; V1 uniform all-seen (109 models, 1920 probes) = 0.5481"
EMBED_MODEL = "sentence-transformers/all-MiniLM-L6-v2"
EPOCHS = 10
BATCH_SIZE = 64
LR = 1e-3
HOLDOUT_FRAC = 0.15
SEEDS = [3, 4]  # extending the existing 0,1,2 (0.5522/0.5613/0.5536, mean=0.5557) to 5 total
PCT = 0.3
...
def main():
    ...
    for seed in SEEDS:
        ...
        ckpt_dir = Path(f"local_checkpoints/embedllm-allseen-encoder-pcaweighted-1800-seed{seed}")
    ...
    out_path = ANALYSIS_DIR / "pcaweighted_allseen_pct30_multiseed_results_1800probes_floor15_seeds3to4.json"
    json.dump(results, open(out_path, "w"), indent=2)

    audcs = [r["audc"] for r in results]
    print("\n" + "=" * 90)
    print(f"CEILING V1.5 (PCA-weighted, 1200 probes) PCT=0.3 COMBINED, EmbedLLM all-seen, {len(SEEDS)} seeds, 111 models")
    print("=" * 90)
    for r in results:
