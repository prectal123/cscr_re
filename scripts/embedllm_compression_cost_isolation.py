"""Isolates the cost of PCA-5 compression from the cost of probe-limiting.

The probe-scale sweep (embedllm_probe_scale_sweep_{wide,uniform}_multiseed.py)
showed AUDC rising with probe count but plateauing/fluctuating well below V2
(full data, 80-dim uncompressed, AUDC~0.587), and never approaching it even
at target=25000 (~12-25k actual probes out of 29,673 total available). Two
confounds are tangled together in that gap: (a) fewer probes = noisier
per-category mean estimates, and (b) PCA-5 compresses 80 raw category dims
down to 5, discarding information regardless of probe count.

This script isolates (b) by comparing two FPs that both use ALL available
data (no probe subsampling) but differ only in compression:
  - embedllm-ceiling        : full data, 80-dim, uncompressed  (== V2 anchor)
  - embedllm-ceiling-pca5   : full data, PCA-5 compressed

Same min(0.3,3)+catfilter loss, same 3-seed protocol, same cached embeddings,
as the scale-sweep scripts -- just no FP building step since both already
exist on disk.
"""
import sys
import time
...
V2_DIR = Path("local_descriptors/embedllm-ceiling")           # full data, 80-dim, uncompressed
PCA5_FULL_DIR = Path("local_descriptors/embedllm-ceiling-pca5")  # full data, PCA-5 compressed
...
def main():
    ...
    def run_point(E, tag):
        ...
        for seed in SEEDS:
            ...
            print(f"  [{tag} seed={seed}] AUDC={m['audc']:.4f} Peak={m['peak']:.4f} QNC={m['qnc']:.4f} "
        ...
        print(f"  [{tag}] MEAN AUDC={audcs.mean():.4f} (std={audcs.std():.4f}) "
              f"Peak={peaks.mean():.4f} (std={peaks.std():.4f})", flush=True)
        return {"audc_mean": float(audcs.mean()), "audc_std": float(audcs.std()),
    ...
    print(f"\n{'#'*70}\nV2: full data, 80-dim, UNCOMPRESSED\n{'#'*70}", flush=True)
    E_v2 = np.stack([np.load(V2_DIR / f"{m}.npy") for m in models]).astype(np.float32)
    results["v2_full_uncompressed"] = {"dim": int(E_v2.shape[1]), **run_point(E_v2, "V2-uncompressed")}

    print(f"\n{'#'*70}\nFull data, PCA-5 COMPRESSED\n{'#'*70}", flush=True)
    E_pca5 = np.stack([np.load(PCA5_FULL_DIR / f"{m}.npy") for m in models]).astype(np.float32)
    results["full_pca5_compressed"] = {"dim": int(E_pca5.shape[1]), **run_point(E_pca5, "full-PCA5")}

    out_path = ANALYSIS_DIR / "compression_cost_isolation_results.json"
    json.dump(results, open(out_path, "w"), indent=2)

    print("\n" + "=" * 90)
    print("COMPRESSION COST ISOLATION (min(0.3,3), floor=1, 3 seeds, all-seen, FULL DATA BOTH ARMS)")
    print("=" * 90)
    print(f"  V2 (80-dim, uncompressed):  AUDC={results['v2_full_uncompressed']['audc_mean']:.4f} "
          f"(std={results['v2_full_uncompressed']['audc_std']:.4f})")
    print(f"  Full data, PCA-5:           AUDC={results['full_pca5_compressed']['audc_mean']:.4f} "
          f"(std={results['full_pca5_compressed']['audc_std']:.4f})")
    gap = results['v2_full_uncompressed']['audc_mean'] - results['full_pca5_compressed']['audc_mean']
    print(f"  Compression-only gap: {gap:.4f}")
    print(f"\nSaved -> {out_path}")
