"""Rebuild the lightweight-20 Perplexity FP using a HEAVIER cross-entropy
scorer (gpt2-large, 774M params, ~6x GPT-2's 124M) instead of plain gpt2.
Mentor feedback: using a small/weak GPT-2 as the Perplexity judge while
Ceiling FP effectively uses ground-truth benchmark scores (a "perfect judge")
is an unfair comparison -- Perplexity's weak showing all session could be a
scorer-capacity artifact, not a methodology-level limitation. Same 528
probes as the original build (controls for everything except scorer size).
"""
import json
import pickle
...
OUT_DIR = DATA_DIR / "perplexity_gpt2large"
CE_MODEL_NAME = "gpt2-large"
...
def main():
    print(f"DEVICE: {DEVICE}  SCORER: {CE_MODEL_NAME}", flush=True)
    ...
    print(f"\nPerplexity FP (gpt2-large) lite20: pairwise cosine sim: mean={off.mean():.4f} std={off.std():.4f} "
