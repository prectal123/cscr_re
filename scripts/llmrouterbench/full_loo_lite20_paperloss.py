"""Full 20-fold parametric LOO using the LITERAL CSCR paper loss
(cost_spectrum_info_nce, Eq.8, n_bands=5) instead of the project's simplified
cost_info_nce -- re-runs the lightweight-20 pool's headline Ceiling FP vs
Perplexity comparison with only the loss function swapped (everything else
identical: same descriptors, same architecture, same beta=1.0 load-balancing,
same seed=0). All outputs go to a SEPARATE directory
(local_descriptors/llmrouterbench_lite20_paperloss/) so nothing mixes with
the cost_info_nce-based results.
"""
import json
import random
import sys
...
OUT_PATH = OUT_DIR / f"full_loo_paperloss_results_seed{SEED}.json"


def main():
    print(f"DEVICE: {base.DEVICE}  LOSS: cost_spectrum_info_nce (paper Eq.8, n_bands={loo.N_BANDS})", flush=True)
    ...
    for fp_name, desc_dir in [("Perplexity", loo.PERP_DIR), ("Ceiling", loo.CEILING_DIR)]:
        ...
        print(f"\n--- {fp_name} summary (paper loss, seed={SEED}) --- mean AUC: {mean_auc:.4f}", flush=True)
