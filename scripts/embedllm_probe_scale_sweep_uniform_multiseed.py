...
pct=0.3), UNIFORM per-category allocation (not PCA-importance-weighted),
from very few probes up to near the full data ceiling (train.csv has
29,673 unique prompts total, 80 categories, per-category availability
ranging 75-3454 -- confirmed by direct query).

This replaces the PCA-importance-weighted allocation used in
embedllm_probe_scale_sweep_wide_multiseed.py. That version plateaued hard:
target=15000 only delivered 10370 actual probes, target=25000 only
delivered 12441 -- because sqrt(importance)-weighted allocation
concentrates probes into a handful of high-importance categories
(medmcqa, mathqa, ...) that saturate their own true data availability
quickly, while low-importance categories stay starved regardless of how
high the nominal target goes. Uniform allocation (same quota per
category, clipped to that category's own true size) doesn't have this
problem -- raising the quota keeps helping every category until each
hits its own ceiling, so actual delivered probes should track target much
more closely all the way up toward the full 29,673.
...
# 29,673 unique prompts total in train.csv (Set A) -- 25000 is close to the
# practical ceiling given per-category availability (75-3454). Starts at
# 1800 (the established "official" fairness-test probe count) -- dropped
# the smaller points (96/300/800) per user request: too noisy, not a good
# basis for showing a scalability trend.
TOTAL_SWEEP = [1800, 4000, 8000, 15000, 25000]
...
def main():
    ...
    for target in TOTAL_SWEEP:
        ...
        fp_dir = Path(f"local_descriptors/embedllm-ceiling-scalesweep-uniform-minpctcap3-{target}-pca5")
    ...
    point2 = run_point(E2, "V2-anchor", "V2 (full data)")
    results["V2"] = {"actual_probes": None, "dim": int(E2.shape[1]), **point2}

    out_path = ANALYSIS_DIR / "probe_scale_sweep_uniform_multiseed_results.json"
    ...
    print(f"  V2 (full data): AUDC={results['V2']['audc_mean']:.4f} (std={results['V2']['audc_std']:.4f})")
    print(f"\nSaved -> {out_path}")
