"""Mentor feedback: min-pos (K=1) generalized to min-top-K (K=3) -- average
the K nearest positive candidates' squared error instead of just the single
nearest. K=1 reproduces min-pos exactly; K=all reproduces vanilla GRPO
(outlier-drag-prone). Quick test on EmbedLLM's main V2 Ceiling FP
(local_descriptors/embedllm-ceiling, 80-dim, category-aggregated, full
data) -- the same FP behind the established all-seen K=1 result (mean
0.5652, PROGRESS.md 22.x), for direct comparability.

Reuses embedllm_pcaweighted_192_allseen_multiseed.py's fast cached-embedding
training pipeline, just swaps the loss function and FP directory.
"""
import json
import sys
import time
...
FP_DIR = Path("local_descriptors/embedllm-ceiling")  # V2, 80-dim, main established FP
ANALYSIS_DIR = Path("local_descriptors/embedllm-analysis")
REFERENCE_NOTE = "V2 all-seen K=1 (min-pos) = 0.5652"
...
def train_fast(seed, cls_all, targets, masks, E_t, hidden_size, tag):
    ...
    for ep in range(EPOCHS):
        ...
        for start in range(0, n_train, BATCH_SIZE):
            ...
            cos_sim = q @ E_t.T
            loss = mintopk_loss(cos_sim, tgt_tr[idx], msk_tr[idx])
            loss.backward()
            opt.step()
            opt.zero_grad()
...
def main():
    ...
    results = []
    for seed in SEEDS:
        proj = train_fast(seed, cls_all, targets, masks, E_t, hidden_size, f"mintop{K_TOPK}-seed{seed}")
        with torch.no_grad():
    ...
    out_path = ANALYSIS_DIR / f"mintop{K_TOPK}_allseen_multiseed_results.json"
    json.dump(results, open(out_path, "w"), indent=2)

    audcs = [r["audc"] for r in results]
    print("\n" + "=" * 90)
    print(f"MIN-TOP-{K_TOPK} (EmbedLLM V2 FP, PCT=0.3 catfilter, all-seen, {len(SEEDS)} seeds, {len(models)} models)")
    print("=" * 90)
    for r in results:
