"""Decisive negative control: replace the FP entirely with PURE RANDOM
NOISE vectors (112 models, 80-dim, unit-normalized, seed=123, carrying
ZERO real information about any model's actual capability) and run the
exact same training+eval pipeline (min(0.3,3)+catfilter, all-seen).

Motivation: random probe SELECTION (embedllm_probe96_random_selection_
negcontrol_allseen_multiseed.py) scored AS WELL AS or better than
top-variance selection at 80 probes -- contradicting the "smart selection
explains the good low-probe scores" hypothesis. This test goes all the way:
if even RANDOM NOISE targets (no real data at all) give similarly high
all-seen AUDC, that would show the query encoder is essentially learning a
category->model LOOKUP using the FP only as a set of arbitrary but fixed
"addresses" to route into -- the FP's actual semantic content wouldn't be
doing the work for all-seen. If AUDC craters instead, the FP's real
content does matter.

All-seen only here (models known during training, so lookup-style
memorization is possible) -- a companion unseen run
(embedllm_randomnoise_fp_unseen_multiseed.py) is the real discriminator,
since unseen models can't be "looked up" from training.
"""
import json
import sys
import time
...
PCA5_DIR = Path("local_descriptors/embedllm-ceiling-RANDOMNOISE")  # pure random noise, no real data
ANALYSIS_DIR = Path("local_descriptors/embedllm-analysis")
REFERENCE_NOTE = ("Real FP, 96-probe(80 actual) top-variance = 0.5798; same probes RANDOMLY selected = 0.5909; "
                   "CSCR all-seen = 0.541")
...
def main():
    ...
    results = []
    for seed in SEEDS:
        proj = train_fast(seed, cls_all, targets, masks, E_t, hidden_size, f"randomnoise-fp-allseen-seed{seed}")
        with torch.no_grad():
    ...
    out_path = ANALYSIS_DIR / "randomnoise_fp_allseen_results.json"
    json.dump(results, open(out_path, "w"), indent=2)

    audcs = [r["audc"] for r in results]
    print("\n" + "=" * 90)
    print(f"RANDOM NOISE FP (no real data), EmbedLLM all-seen, {len(SEEDS)} seeds, {len(models)} models")
    print("=" * 90)
    for r in results:
