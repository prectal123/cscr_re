...
MODEL_COUNTS = [5, 10, 20, 30, 50, 75, 111]
...
def main():
    ...
    for n_models in MODEL_COUNTS:
        ...
        audcs = [r["audc"] for r in results[str(n_models)] if np.isfinite(r["audc"])]
        print(f"\n[n_models={n_models}] MEAN AUDC={np.mean(audcs):.4f} (std={np.std(audcs):.4f}) "
              f"n_valid={len(audcs)}/{len(SEEDS)}", flush=True)

        out_path = ANALYSIS_DIR / "modelcount_sweep_fixedprobes_allseen_results.json"
    ...
    print(f"\nSaved -> {ANALYSIS_DIR / 'modelcount_sweep_fixedprobes_allseen_results.json'}")
