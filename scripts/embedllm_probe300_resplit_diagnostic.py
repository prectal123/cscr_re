"""Diagnostic for the target=300 AUDC spike: user's hypothesis is that the
SPECIFIC HF train.csv/test.csv split happens to make the 300-probe subset
(4 probes/category, top-variance) unusually well-aligned with Set B by
sheer coincidence -- not literal leakage (already confirmed zero
overlapping prompt_id/text between train.csv and test.csv), but some kind
of distributional fluke tied to THIS PARTICULAR split.

Test: pool train.csv + test.csv together, draw a FRESH random split (new
seed, different from HF's official one) into New-Set-A (90%) / New-Set-B
(10%) by unique prompt_id. Rebuild probe-limited FPs (uncompressed,
uniform allocation, same nlargest-by-variance selection) using ONLY
New-Set-A, train on New-Set-A, evaluate on New-Set-B. If the 300-spike
disappears or moves under this different split, it confirms the user's
"lucky split" hypothesis. If it persists at n=4/category regardless of
which specific data went into Set A/B, that points to something more
structural about the selection method itself.
"""
...
HOLDOUT_FRAC = 0.15
SEEDS = [0, 1, 2]
RESPLIT_SEED = 777  # fresh split, independent of HF's official train/test split
NEW_SETB_FRAC = 0.10
PCT_CATFILTER = 0.3
PCT_MINLOSS = 0.3
K_CAP = 3
...
def build_probe_sampled_fp_nocompress(df, allocation, models, categories):
...
def main():
    ...
    print(f"pooled: {len(df_all)} rows, {len(all_prompt_ids)} unique prompts", flush=True)
    ...
    print(f"NEW split (resplit_seed={RESPLIT_SEED}): SetA={len(seta_ids)} SetB={len(setb_ids)} prompts "
          f"(overlap check: {len(seta_ids & setb_ids)})", flush=True)
    ...
    print("Building GRPO targets + pct30 catfilter mask (NEW Set A)...", flush=True)
    ...
    # NEW Set B eval set (built directly from pooled df, not load_embedllm)
    name_to_idx = {m: i for i, m in enumerate(models)}
    eval_by_pid = {}
    for pid, grp in df_setb.groupby("prompt_id", sort=False):
        prompt = grp["prompt"].iloc[0]
        lbl_map = {r["model_name"]: r["label"] for _, r in grp.iterrows() if r["model_name"] in name_to_idx}
        if len(lbl_map) >= 2:
            eval_by_pid[pid] = (prompt, lbl_map)
    eval_texts = [v[0] for v in eval_by_pid.values()]
    label_maps = [v[1] for v in eval_by_pid.values()]
    print(f"NEW Set B (eval): {len(eval_texts)} rows", flush=True)
    t0 = time.time()
    ...
    for target in TARGETS:
        ...
        E, n_probes = build_probe_sampled_fp_nocompress(df, allocation, models, categories)
        ...
        seed_metrics = []
        for seed in SEEDS:
            proj = train_fast(seed, cls_all, targets, masks, E_t, hidden_size, f"resplit-{target}-seed{seed}")
            with torch.no_grad():
        ...
        results[str(target)] = {"actual_probes": n_probes, "audc_mean": float(audcs.mean()),
                                 "audc_std": float(audcs.std()),
                                 "per_seed": [float(a) for a in audcs]}

    out_path = Path("local_descriptors/embedllm-analysis/probe300_resplit_diagnostic_results.json")
    json.dump(results, open(out_path, "w"), indent=2)
    print("\n" + "=" * 90)
    print(f"RESPLIT DIAGNOSTIC (new split, seed={RESPLIT_SEED}) -- does target=300 still spike?")
    print("=" * 90)
    for target in TARGETS:
