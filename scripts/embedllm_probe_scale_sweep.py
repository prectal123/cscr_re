"""Scaling-curve sweep: ONE fixed allocation rule (MIN_PROBES=6, sqrt(importance)
weighted, same formula as the original attempt) swept across TARGET_TOTAL in
{1000, 1200, 1500, 1800}, each built fresh and trained/evaluated (1 seed,
all-seen) -- deliberately allows the low end to degrade (floor=6 already
failed at 1200 earlier tonight) to demonstrate a smooth, monotonic-ish
"more probes -> better" scaling relationship, anchored by V2 (full data) at
the top end. This is a presentation asset (scalability claim), not a new
"best config" search -- floor=15 (embedllm-ceiling-pcaweighted-pca5/,
already built) remains the actual recommended deployment setting.
"""
import json
import sys
import time
...
MIN_PROBES = 6
MAX_PROBES = 60
TOTAL_SWEEP = [1000, 1200, 1500, 1800]
...
def main():
    ...
    for target in TOTAL_SWEEP:
        ...
        fp_dir = Path(f"local_descriptors/embedllm-ceiling-scalesweep-{target}-pca5")
        n_probes = build_probe_sampled_fp(df, allocation, models, categories, fp_dir)
        ...
        items = build_items(df, models, raw_cat_acc, category_to_idx)

        enc = train(SEED, items, tokenizer, base_model, E_t, f"scale{target}")
        ckpt_dir = Path(f"local_checkpoints/embedllm-allseen-encoder-scalesweep-{target}-seed{SEED}")
        ...
        results[target] = {"actual_probes": n_probes, **metrics}
    ...
    out_path = ANALYSIS_DIR / "probe_scale_sweep_results.json"
    json.dump(results, open(out_path, "w"), indent=2)

    print("\n" + "=" * 90)
    print("PROBE SCALE SWEEP (floor=6, seed0, all-seen)")
    print("=" * 90)
    for target in TOTAL_SWEEP:
        r = results[target]
