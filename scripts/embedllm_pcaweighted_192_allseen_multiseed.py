...
def train_fast(seed, cls_all, targets, masks, E_t, hidden_size, tag):
    ...
    for ep in range(EPOCHS):
        ...
        for start in range(0, n_train, BATCH_SIZE):
            ...
            cos_sim = q @ E_t.T
            loss = minpos_loss(cos_sim, tgt_tr[idx], msk_tr[idx])
            loss.backward()
            opt.step()
            opt.zero_grad()
...
def main():
    ...
    results = []
    for seed in SEEDS:
        proj = train_fast(seed, cls_all, targets, masks, E_t, hidden_size, f"pct30-192probe-seed{seed}")
        with torch.no_grad():
    ...
    out_path = ANALYSIS_DIR / "pcaweighted_allseen_pct30_multiseed_results_192probes.json"
    json.dump(results, open(out_path, "w"), indent=2)

    audcs = [r["audc"] for r in results]
    print("\n" + "=" * 90)
    print(f"CEILING V1.5 (PCA-weighted, 192 probes) PCT=0.3 COMBINED, EmbedLLM all-seen, {len(SEEDS)} seeds, {len(models)} models")
    print("=" * 90)
    for r in results:
