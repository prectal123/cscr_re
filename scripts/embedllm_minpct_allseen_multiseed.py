"""Follow-up to embedllm_mintopk_allseen_multiseed.py (K=3 fixed count,
mean AUDC=0.5863 vs K=1's 0.5652): percentage-based version instead of a
fixed count -- average the top pct=0.3 closest positive candidates
(per-row K = ceil(0.3 * n_positives), same style as the catfilter's
top50pct/pct30 selection, but applied to the min-pos LOSS AGGREGATION
itself rather than the catfilter's pre-loss masking step). A query with 10
positive candidates uses top-3; a query with 2 uses top-1 (ceil(0.3*2)=1).

Same EmbedLLM V2 FP (80-dim, main established FP), same pct=0.3 catfilter
upstream, same fast cached pipeline as the K=3 test.
"""
import json
import sys
import time
...
REFERENCE_NOTE = "V2 all-seen K=1 (min-pos) = 0.5652; min-top-3 (fixed count) = 0.5863"
...
def minpct_loss(cos_sim, target, mask, pct=PCT_MINLOSS):
    """Like mintopk_loss but K is a per-row percentage of available
    positives (ceil(pct * n_pos)) instead of a fixed count."""
    ...
    has_pos = n_pos > 0
    k_eff = torch.clamp(torch.ceil(n_pos.float() * pct).long(), min=1)
...
def train_fast(seed, cls_all, targets, masks, E_t, hidden_size, tag):
    ...
    for ep in range(EPOCHS):
        ...
        for start in range(0, n_train, BATCH_SIZE):
            ...
            cos_sim = q @ E_t.T
            loss = minpct_loss(cos_sim, tgt_tr[idx], msk_tr[idx])
            loss.backward()
            opt.step()
            opt.zero_grad()
...
def main():
    ...
    results = []
    for seed in SEEDS:
        proj = train_fast(seed, cls_all, targets, masks, E_t, hidden_size, f"minpct{PCT_MINLOSS}-seed{seed}")
        with torch.no_grad():
    ...
    out_path = ANALYSIS_DIR / f"minpct{PCT_MINLOSS}_allseen_multiseed_results.json"
    json.dump(results, open(out_path, "w"), indent=2)

    audcs = [r["audc"] for r in results]
    print("\n" + "=" * 90)
    print(f"MIN-PCT-{PCT_MINLOSS} (EmbedLLM V2 FP, catfilter pct={PCT_CATFILTER}, all-seen, {len(SEEDS)} seeds, {len(models)} models)")
    print("=" * 90)
    for r in results:
