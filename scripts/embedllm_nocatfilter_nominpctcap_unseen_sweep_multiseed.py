...
def build_items(df, models, raw_cat_acc, category_to_idx, pct):
    # catfilter REMOVED (per user request) -- mask = validity only, no
    # positive-demotion by per-category raw accuracy. pct/raw_cat_acc/
    # category_to_idx args kept for call-signature compatibility, unused.
...
def plain_mse_loss(cos_sim, target, mask):
    """No catfilter, no min(0.3,3) top-k emphasis -- plain masked MSE over
    every (cos_sim, target) pair, positives and negatives treated equally."""
...
def train_fast(seed, cls_all, targets, masks, E_t, hidden_size, tag):
    ...
    for ep in range(EPOCHS):
        ...
        for start in range(0, n_train, BATCH_SIZE):
            ...
            cos_sim = q @ E_t.T
            loss = plain_mse_loss(cos_sim, tgt_tr[idx], msk_tr[idx])
            loss.backward()
            opt.step()
            opt.zero_grad()
...
def main():
    ...
    out_path = ANALYSIS_DIR / "probe_scale_sweep_unseen_nocatfilter_nominpctcap_results.json"
