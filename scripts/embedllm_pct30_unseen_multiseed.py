"""Same pct=0.3 catfilter + min-pos combined recipe that just gave a clean,
tight all-seen win (mean=0.5652, std=0.0005, 3/3 vs CSCR 0.541, beating
both min-pos-alone 0.5587 and top50pct-combined 0.5581) -- now tested on
the unseen protocol (3 seeds), to see if the same pct=0.3 sweet spot holds
there too, vs the reference top50pct-combined unseen mean (0.5163, seed0-2).
"""
import json
import sys
import time
...
REFERENCE = {"combined_top50pct_seed0to2": 0.5163}
...
def main():
    ...
    for seed in SEEDS:
        ...
        print(f"\n{'='*60}\nSEED {seed}: seen={len(seen_models)}\n{'='*60}", flush=True)
        enc = train(seed, items, tokenizer, base_model, E_seen_t, f"pct30-unseen-seed{seed}")
        ckpt_dir = Path(f"local_checkpoints/embedllm-newllm-encoder-pct30-seed{seed}")
        ckpt_dir.mkdir(parents=True, exist_ok=True)
        enc.save(ckpt_dir)
        r = fast_eval_unseen(str(split_path), str(unseen_dir), str(ckpt_dir), label=f"pct30-unseen-seed{seed}")
    ...
    out_path = ANALYSIS_DIR / "unseen_catfilter_pct30_multiseed_results.json"
    json.dump(results, open(out_path, "w"), indent=2)

    audcs = [r["audc"] for r in results]
    print("\n" + "=" * 90)
    print(f"PCT=0.3 CATFILTER + MIN-POS (COMBINED), EmbedLLM unseen, {len(SEEDS)} seeds")
    print("=" * 90)
    for r in results:
    ...
    print(f"reference: combined-top50pct(seed0-2 mean)={REFERENCE['combined_top50pct_seed0to2']}")
    print(f"\nSaved -> {out_path}")
