...
def main():
    ...
    out_path = ANALYSIS_DIR / "pcaweighted_allseen_minpctcap3_multiseed_results_v15_1800probes_seeds3to9.json"
    json.dump(results, open(out_path, "w"), indent=2)

    audcs = [r["audc"] for r in results]
    print("\n" + "=" * 90)
    print(f"CEILING V1.5 (PCA-weighted, 1800 probes, floor=15) MIN(0.3,3) COMBINED, EmbedLLM all-seen, {len(SEEDS)} seeds, {len(models)} models")
    print("=" * 90)
    for r in results:
