"""Decisive negative control (UNSEEN side): same idea as
embedllm_randomnoise_fp_allseen_multiseed.py -- FP replaced entirely with
PURE RANDOM NOISE vectors (embedllm-ceiling-RANDOMNOISE, 112 models,
80-dim, unit-normalized, seed=123, zero real information) -- but here
under the UNSEEN protocol.

This is the REAL discriminator. All-seen COULD in principle score well
with noise vectors if the query encoder just memorizes a category->model
lookup using the noise vectors as arbitrary fixed addresses (the
seen-model identities are known during training). But UNSEEN models are
by definition never seen during training -- there is no way to "look up"
a fixed address for a model whose noise vector the encoder never
optimized against. If unseen AUDC with noise vectors stays anywhere near
the real-FP unseen numbers (~0.52), that is a serious red flag about the
whole eval pipeline. If it craters to near/below CSCR-unseen or random
baseline, that CONFIRMS the real FP's content is what's carrying unseen
generalization, and the all-seen anomalies are explained by benign
lookup-style memorization specific to all-seen.
"""
import json
import sys
import time
...
POINTS = [
    ("random-noise-fp", "embedllm-ceiling-RANDOMNOISE"),
]
...
def main():
    ...
    out_path = ANALYSIS_DIR / "randomnoise_fp_unseen_results.json"
