"""MIXED-POOL unseen eval: same seen-only training as the standard unseen
protocol, but at EVAL time the routing candidate pool is opened up to
ALL 111 models (seen + unseen) instead of being restricted to unseen-only.

Motivation: in the standard unseen eval, load_embedllm("test",
candidates=unseen) DROPS every seen model's label from label_map entirely
-- so (a) a query where NO unseen model is correct is an unconditional
failure even if 9 seen models would have nailed it, and (b) a query where
only 1 of 6 correct models happens to be unseen means success requires
finding that exact needle among ~35-40 unseen candidates, ignoring the
5 easier seen options entirely. This makes the standard eval harsher in a
way that's an artifact of the eval design, not necessarily a reflection
of true generalization quality.

This script tests the more realistic "deployed fleet" scenario: the
already-seen-only-trained projection head now faces the FULL candidate
pool (seen models it was trained to recognize + the never-before-seen
ones), same as a real router serving a fleet that just had new experts
added to it. Does it still find the unseen models when they're the right
answer, even with tempting familiar alternatives available? Uses the
SAME official 1800-probe uncompressed FP as the headline unseen result,
so AUDC is directly comparable to both the all-seen number (0.5787) and
the unseen-only-candidates number (0.5232).
"""
import json
import sys
import time
...
# Just the official headline FP -- same one used for both the all-seen
# (0.5787) and unseen-only-candidates (0.5232) numbers.
POINTS = [
...
def main():
    ...
    for seed in SEEDS:
        ...
        mixed_models = seen_models + unseen_models  # FULL candidate pool at eval time
        dataset = load_embedllm("test", candidates=mixed_models)
        eval_texts = [ex["prompt"] for ex in dataset]
        label_maps = [ex["label_map"] for ex in dataset]
        t0 = time.time()
        cls_setB_mixed = precompute_cls(eval_texts, tokenizer, base_model)
        print(f"  Set B mixed-pool ({len(eval_texts)} rows, {len(mixed_models)} candidate models) "
              f"cached in {time.time()-t0:.1f}s", flush=True)
        ...
        seed_cache[seed] = dict(seen_models=seen_models, unseen_models=unseen_models,
                                 ...
                                 cls_setB_mixed=cls_setB_mixed, label_maps=label_maps)
    ...
    for label, fp_dir_name in POINTS:
        ...
        for seed in SEEDS:
            c = seed_cache[seed]
            seen_models, unseen_models, mixed_models = c["seen_models"], c["unseen_models"], c["mixed_models"]
            ...
            # MIXED pool: candidate FP matrix now has BOTH seen and unseen models
            E_mixed = np.stack([np.load(fp_dir / f"{m}.npy") for m in mixed_models]).astype(np.float32)
            E_mixed_norm = E_mixed / (np.linalg.norm(E_mixed, axis=1, keepdims=True) + 1e-12)
            costs_mixed = np.array([compute_cost(m, 0, cost_type="n_params") for m in mixed_models], dtype=np.float32)

            with torch.no_grad():
                embeds = F.normalize(proj(torch.from_numpy(c["cls_setB_mixed"]).float().to(DEVICE)),
                                      dim=-1).cpu().numpy()
            sims = embeds @ E_mixed_norm.T
            knn_costs, knn_accs = knn_curve(sims, mixed_models, c["label_maps"], costs_mixed, LAM_LIST)
            m = audc_qnc_peak(knn_costs, knn_accs)
            seed_metrics.append(m)
            seed_costs.append(knn_costs)
            seed_accs.append(knn_accs)

            # sharper diagnostic (user's request): bucket EVERY query by which side
            # holds the correct answer(s), then measure top-1 (pure-similarity,
            # lambda=0, no cost/bandit) accuracy WITHIN each bucket separately.
            # The critical bucket is "unseen_only": no seen model is correct at
            # all, so a seen-biased router (classification-collapse memorization)
            # would be expected to miss these even though it still has a real
            # unseen model available that IS correct.
            unseen_set = set(unseen_models)
            seen_set = set(seen_models)
            top1_idx = np.argmax(sims, axis=1)
            bucket_total = {"seen_only": 0, "unseen_only": 0, "both": 0, "neither": 0}
            bucket_correct = {"seen_only": 0, "unseen_only": 0, "both": 0, "neither": 0}
            for i in range(sims.shape[0]):
                lm = c["label_maps"][i]
                has_seen = any(lm.get(mm, 0) == 1 for mm in seen_set)
                has_unseen = any(lm.get(mm, 0) == 1 for mm in unseen_set)
                bucket = ("both" if (has_seen and has_unseen) else
                          "seen_only" if has_seen else
                          "unseen_only" if has_unseen else "neither")
                chosen = mixed_models[top1_idx[i]]
                correct = lm.get(chosen, 0) == 1
                bucket_total[bucket] += 1
                bucket_correct[bucket] += int(correct)

            bucket_str = " ".join(
                f"{b}={bucket_correct[b]}/{bucket_total[b]}"
                f"({100*bucket_correct[b]/max(bucket_total[b],1):.1f}%)"
                for b in ["seen_only", "unseen_only", "both", "neither"]
            )
            ...
            print(f"    top1-by-bucket: {bucket_str}", flush=True)
    ...
    out_path = ANALYSIS_DIR / "unseen_mixedpool_1800_results.json"
    json.dump(results, open(out_path, "w"), indent=2)

    print("\n" + "=" * 90)
    print("MIXED-POOL UNSEEN EVAL -- seen+unseen candidates, official 1800-probe FP")
    print("=" * 90)
