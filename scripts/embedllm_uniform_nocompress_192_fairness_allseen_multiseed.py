"""Fairness point for Final_Result_Summary2.MD: shrink Ceiling to CSCR's
original probe budget (192 probes, matching CSCR's Perplexity FP probe
count), on the CURRENT official pipeline (uncompressed 80-dim FP, uniform
per-category allocation, min(0.3,3)+catfilter). v1's equivalent (§1.4 of
Final_result_Summary.MD) used PCA-weighted allocation + PCA-5 compression
(0.5436) -- this reruns it under the now-official uncompressed+uniform
recipe for direct comparability with the 1.1/1.2 headline numbers.

Single sweep point (192), 3 seeds. Reuses the exact same FP-building code
path as the probe-scale-sweep scripts (uniform allocation, no PCA
...
TOTAL_SWEEP = [192]  # CSCR's original probe budget -- single fairness point
...
def main():
    ...
    out_path = ANALYSIS_DIR / "uniform_nocompress_192_fairness_allseen_results.json"
    json.dump(results, open(out_path, "w"), indent=2)

    print("\n" + "=" * 90)
    print("FAIRNESS: 192-probe, uncompressed, uniform allocation, all-seen, 3 seeds")
    print("=" * 90)
    for target in TOTAL_SWEEP:
