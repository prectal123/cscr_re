"""Negative control: user is unsettled that even 80-160 probes (~1-2 per
category) give All-seen/Unseen AUDC well above CSCR. Is this a genuine
consequence of top-VARIANCE probe selection picking maximally
discriminative prompts, or is something else going on?

Test: same probe count (target=96, same uniform per-category quota as the
official low-range sweep point), but select probes RANDOMLY within each
category instead of nlargest(n,"var"). If random selection craters
performance, the high scores at low probe counts are explained by smart
selection, not an artifact. If random selection ALSO scores suspiciously
well, that points to something else needing investigation.

Same uniform per-category quota, uncompressed 80-dim FP, min(0.3,3)+
catfilter loss, 3 seeds, all-seen -- everything held constant except the
WITHIN-category selection rule (nlargest(var) -> random).
...
TOTAL_SWEEP = [96]  # extreme low end -- most dramatic effect expected if hypothesis correct
...
def build_probe_sampled_fp(df, allocation, models, categories, out_dir, random_seed=None):
    ...
    for cat, grp in per_prompt.groupby("category"):
        ...
        if rng is not None:
            n = min(n, len(grp))
            top = grp.sample(n=n, random_state=rng)
        else:
...
def main():
    ...
    RANDOM_SELECTION_SEED = 42  # fixed, distinct from training seeds -- controls WHICH random probes get picked

    for target in TOTAL_SWEEP:
        print(f"\n{'#'*70}\nTARGET_TOTAL={target} (RANDOM probe selection, not top-variance)\n{'#'*70}", flush=True)
        allocation, actual_total = solve_allocation_uniform(cat_sizes, categories, target, MIN_PROBES)
        fp_dir = Path(f"local_descriptors/embedllm-ceiling-scalesweep-uniform-nocompress-RANDOMSEL-minpctcap3-{target}")
        n_probes = build_probe_sampled_fp(df, allocation, all_models, categories, fp_dir, random_seed=RANDOM_SELECTION_SEED)
        print(f"  built FP with {n_probes} actual RANDOMLY-selected probes -> {fp_dir}", flush=True)

        E = np.stack([np.load(fp_dir / f"{m}.npy") for m in models]).astype(np.float32)
        point = run_point(E, f"randomsel{target}", f"target={target} actual={n_probes} RANDOM")
        results[str(target)] = {"actual_probes": n_probes, "dim": int(E.shape[1]), **point}

    out_path = ANALYSIS_DIR / "probe96_random_selection_negcontrol_allseen_results.json"
    json.dump(results, open(out_path, "w"), indent=2)

    print("\n" + "=" * 90)
    print("NEGATIVE CONTROL: RANDOM probe selection (not top-variance), min(0.3,3), 3 seeds, all-seen")
    print("=" * 90)
    for target in TOTAL_SWEEP:
        r = results[str(target)]
        print(f"  target={target:>6} actual={r['actual_probes']:>6}: AUDC={r['audc_mean']:.4f} (std={r['audc_std']:.4f})")
    print(f"  (compare vs top-variance selection at target=96: AUDC=0.5798, std=0.0004)")
    print(f"\nSaved -> {out_path}")
