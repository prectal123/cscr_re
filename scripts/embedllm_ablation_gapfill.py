"""Fills the remaining gaps for a clean pct=0.3-based ablation table
(RESULTS_SUMMARY.md sections 2.2/2.3):

  1. EmbedLLM all-seen, vanilla GRPO (no catfilter, no min-pos) -- never run.
  2. EmbedLLM all-seen, catfilter(pct=0.3) ALONE (mean aggregation, not
     min-pos) -- never run, only combined(pct=0.3, with min-pos) exists.
  3. EmbedLLM unseen, catfilter(pct=0.3) ALONE (mean aggregation) -- never
     run, only the old Top-2 version and combined(pct=0.3) exist.

3 seeds (0,1,2) each, matching this session's convention. Reuses the exact
training/eval code already verified in embedllm_catfilter_pct_sweep.py and
embedllm_combined_multiseed.py -- only the loss function (plain MSE vs
minpos) and which config runs on which protocol differ.
"""
import json
import sys
import time
...
def build_items(df, models, raw_cat_acc, category_to_idx, apply_catfilter):
    """apply_catfilter=False -> vanilla (unfiltered) mask. True -> pct=0.3 mask."""
...
def run_allseen(tag, apply_catfilter, models, costs, df, categories, category_to_idx,
                 tokenizer, base_model, E_t, E_norm, texts, label_maps, raw_cat_acc):
    results = []
    items = build_items(df, models, raw_cat_acc, category_to_idx, apply_catfilter)
    for seed in SEEDS:
        enc = train(seed, items, tokenizer, base_model, E_t, f"allseen-{tag}-seed{seed}")
        ...
        print(f"  [allseen-{tag} seed={seed}] AUDC={metrics['audc']:.4f} Peak={metrics['peak']:.4f} "
              f"({'BEATS' if metrics['audc'] > CSCR_ALLSEEN else 'below'} CSCR {CSCR_ALLSEEN})", flush=True)
    return results


def run_unseen_catfilter(tokenizer, base_model, df, categories, category_to_idx):
    results = []
    for seed in SEEDS:
        ...
        items = build_items(df, seen_models, raw_cat_acc, category_to_idx, apply_catfilter=True)

        enc = train(seed, items, tokenizer, base_model, E_seen_t, f"unseen-catfilterpct30-seed{seed}")
        ckpt_dir = Path(f"local_checkpoints/embedllm-newllm-encoder-catfilterpct30alone-seed{seed}")
        ckpt_dir.mkdir(parents=True, exist_ok=True)
        enc.save(ckpt_dir)
        r = fast_eval_unseen(str(split_path), str(unseen_dir), str(ckpt_dir), label=f"unseen-catfilterpct30-seed{seed}")
        results.append({"seed": seed, **r["knn"]})
        print(f"  [unseen-catfilter(pct0.3) seed={seed}] AUDC={r['knn']['audc']:.4f} Peak={r['knn']['peak']:.4f} "
...
def main():
    ...
    print("\n" + "#" * 70 + "\nALL-SEEN: VANILLA GRPO\n" + "#" * 70, flush=True)
    all_results["allseen_vanilla"] = run_allseen("vanilla", False, models, costs, df, categories, category_to_idx,
                                                   tokenizer, base_model, E_t, E_norm, texts, label_maps, raw_cat_acc)

    print("\n" + "#" * 70 + "\nALL-SEEN: CATFILTER(pct=0.3) ALONE\n" + "#" * 70, flush=True)
    all_results["allseen_catfilter_pct30"] = run_allseen("catfilterpct30", True, models, costs, df, categories, category_to_idx,
                                                            tokenizer, base_model, E_t, E_norm, texts, label_maps, raw_cat_acc)

    print("\n" + "#" * 70 + "\nUNSEEN: CATFILTER(pct=0.3) ALONE\n" + "#" * 70, flush=True)
    all_results["unseen_catfilter_pct30"] = run_unseen_catfilter(tokenizer, base_model, df, categories, category_to_idx)

    out_path = ANALYSIS_DIR / "ablation_gapfill_results.json"
    json.dump(all_results, open(out_path, "w"), indent=2)

    print("\n" + "=" * 90)
    print("ABLATION GAP-FILL SUMMARY")
    print("=" * 90)
    for key, rows in all_results.items():
        audcs = [r["audc"] for r in rows]
        print(f"{key}: " + " / ".join(f"{a:.4f}" for a in audcs) + f"  mean={np.mean(audcs):.4f} (std={np.std(audcs):.4f})")
    print(f"\nSaved -> {out_path}")
