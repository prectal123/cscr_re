...
PCA5_DIR = Path("local_descriptors/embedllm-ceiling-scalesweep-uniform-nocompress-minpctcap3-1800")  # official 1800-probe, uncompressed, uniform allocation
ANALYSIS_DIR = Path("local_descriptors/embedllm-analysis")
REFERENCE_NOTE = ("Official 1800-probe all-seen WITH catfilter = 0.5787 (std 0.0014); "
                   "CSCR all-seen = 0.541")
...
def main():
    ...
    results = []
    for seed in SEEDS:
        proj = train_fast(seed, cls_all, targets, masks, E_t, hidden_size, f"ablation-nocatfilter-1800-seed{seed}")
        with torch.no_grad():
    ...
    out_path = ANALYSIS_DIR / "uniform_nocompress_1800_ablation_nocatfilter_allseen_results.json"
    json.dump(results, open(out_path, "w"), indent=2)

    audcs = [r["audc"] for r in results]
    print("\n" + "=" * 90)
    print(f"ABLATION: catfilter OFF, official 1800-probe uncompressed uniform FP, EmbedLLM all-seen, {len(SEEDS)} seeds, {len(models)} models")
    print("=" * 90)
    for r in results:
