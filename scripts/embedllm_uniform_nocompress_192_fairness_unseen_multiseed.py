"""Fairness point, UNSEEN protocol: EmbedLLM at CSCR's own original probe
budget (192 target / 160 actual, uniform allocation, uncompressed 80-dim
FP) -- the Unseen-side counterpart to
embedllm_uniform_nocompress_192_fairness_allseen_multiseed.py (which only
covered All-seen). Reuses the FP dir already built by that script
(embedllm-ceiling-scalesweep-uniform-nocompress-minpctcap3-192).
...
Adapted from embedllm_probe_scale_sweep_unseen_multiseed_withcurves.py,
trimmed to the single 192 point.
"""
import json
import sys
import time
...
def main():
    ...
    out_path = ANALYSIS_DIR / "uniform_nocompress_192_fairness_unseen_results.json"
    json.dump(results, open(out_path, "w"), indent=2)

    print("\n" + "=" * 90)
    print("FAIRNESS: 192-probe, uncompressed, uniform allocation, UNSEEN, 3 seeds")
    print("=" * 90)
