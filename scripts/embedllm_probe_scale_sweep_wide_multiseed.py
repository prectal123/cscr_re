...
pct=0.3), PCA-weighted allocation (floor=1), from very few probes up to
near the full data ceiling (train.csv has 29,673 unique prompts total,
80 categories, per-category availability ranging 75-3454 -- confirmed by
direct query). MAX_PROBES raised from 60 to 3500 (just above the largest
category's availability, 3454) so high TARGET_TOTAL values aren't
artificially capped -- solve_allocation's clip(min_n, max_n) only binds
when actually needed, so this doesn't change behavior at low targets.
...
def main():
    ...
    out_path = ANALYSIS_DIR / "probe_scale_sweep_wide_multiseed_results.json"
