...
REFERENCE_NOTE = "V2 all-seen K=1 (min-pos) = 0.5652; min-top-3 (fixed count) = 0.5863; min-pct30 (uncapped) = 0.5858"
...
def main():
    ...
    results = []
    for seed in SEEDS:
        proj = train_fast(seed, cls_all, targets, masks, E_t, hidden_size, f"minpct{PCT_MINLOSS}cap{K_CAP}-seed{seed}")
        with torch.no_grad():
    ...
    out_path = ANALYSIS_DIR / f"minpct{PCT_MINLOSS}cap{K_CAP}_allseen_multiseed_results.json"
