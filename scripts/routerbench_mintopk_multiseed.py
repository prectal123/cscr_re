"""Mentor feedback: instead of min-pos (K=1, match only the single nearest
positive), average the K nearest positives (K=3) -- interpolates between
min-pos (K=1, most outlier-drag-resistant) and vanilla GRPO (K=all, most
outlier-drag-prone). Quick test on RouterBench's existing Ceiling V1.5 FP
(86-dim, 1761 probes, already built) using the fast cached-embedding
pipeline, same 3 seeds/all-seen as the K=1 result (0.7282 mean, PROGRESS.md
24.3) for direct comparability.
"""
import json
import sys
import time
...
def mintopk_loss(cos_sim, target, mask, k=K_TOPK):
    """Same as minpos_loss but averages the K smallest positive errors
    instead of taking the single minimum. K=1 reproduces minpos_loss exactly."""
    ...
    has_pos = n_pos > 0
    k_eff = torch.clamp(n_pos, max=k)  # per-row K, can't exceed available positives
...
def train_mintopk_fast(seed, cls_tr, targets, masks, E_t, hidden_size, tag):
    n = cls_tr.shape[0]
    ...
    for ep in range(EPOCHS):
        ...
        for start in range(0, n_train, BATCH_SIZE):
            ...
            cos_sim = q @ E_t.T
            loss = mintopk_loss(cos_sim, tgt_t[idx], msk_t[idx])
            loss.backward()
            opt.step()
            opt.zero_grad()
...
def main():
    ...
    print(f"\nmintop{K_TOPK}-ceiling-v15 (FP dim={E.shape[1]}, dir={CEILING_V15_DIR})", flush=True)

    results = []
    for seed in SEEDS:
        proj = train_mintopk_fast(seed, cls_tr, targets, masks, E_t, hidden_size, f"seed{seed}-mintop{K_TOPK}")
        ...
        print(f"  [seed={seed} mintop{K_TOPK}] AUDC={knn_metrics['audc']:.4f} Peak={knn_metrics['peak']:.4f} "
    ...
    out_path = Path("local_descriptors/routerbench-analysis/mintopk_multiseed_results.json")
    json.dump({f"mintop{K_TOPK}": results}, open(out_path, "w"), indent=2)

    audcs = [r["knn"]["audc"] for r in results]
    print("\n" + "=" * 90)
    print(f"MIN-TOP-{K_TOPK} (RouterBench Ceiling V1.5, seeds {SEEDS}, all-seen) vs K=1 mean=0.7282 (24.3) vs CSCR {CSCR_ROUTERBENCH}")
    print("=" * 90)
    print(f"mintop{K_TOPK}: " + " / ".join(f"{a:.4f}" for a in audcs) +
